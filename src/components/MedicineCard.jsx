...
export const MedicineCard = ({ medicine, onEdit, onDelete, onShare, onReminder, hasReminder = false }) => {
  ...
  return (
    <div className={`group relative bg-white dark:bg-slate-900 rounded-2xl border ${s.border} dark:border-slate-700/80 shadow-[0_1px_0_rgba(15,23,42,0.04)] hover:shadow-[0_8px_24px_-12px_rgba(15,23,42,0.18)] hover:-translate-y-0.5 transition-all duration-200 p-5 flex flex-col`}>
      {/* Status accent bar */}
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
          {medicine.canEdit !== false ? (
            ...
            <span className="text-[11px] text-slate-400 self-center px-1">{medicine.ownerName}</span>
          )}
        </div>
      </div>

      {/* Name */}
      <h3 className="mt-3 text-[17px] font-semibold text-slate-900 dark:text-slate-100 leading-snug tracking-tight">{medicine.name}</h3>
      {medicine.quantity && <div className="mt-0.5 text-[13px] text-slate-500 dark:text-slate-400">{medicine.quantity}</div>}

      {/* Ingredients */}
      {ingredients.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {ingredients.map((ing, i) => (
            <span key={i} className="inline-flex items-center gap-1 text-[11.5px] bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2 py-0.5 rounded-md ring-1 ring-slate-200/80 dark:ring-slate-700">
      ...
      {/* Notes */}
      {medicine.notes && (
        <div className="mt-3 text-[12.5px] text-slate-500 dark:text-slate-400 line-clamp-2 italic">"{medicine.notes}"</div>
      )}

      {/* Footer */}
      <div className="mt-4 pt-3 border-t border-dashed border-slate-200 dark:border-slate-700 flex items-center justify-between text-[13px]">
        <div className="flex items-center gap-1.5">
          <Calendar size={14} className="text-slate-400" />
          <span className={st.key === 'expired' ? 'text-rose-700 font-semibold' : 'text-slate-700 dark:text-slate-300 font-medium'}>
        ...
        {st.daysLeft != null && st.key !== 'expired' && (
          <span className={`inline-flex items-center gap-1 text-[12px] tabular-nums ${
            st.key === 'warning' ? 'text-amber-700' : st.key === 'soon' ? 'text-sky-700' : 'text-slate-500 dark:text-slate-400'
          }`}>
