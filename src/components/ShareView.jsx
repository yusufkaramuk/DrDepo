      ...
      <div className="w-full max-w-md">
        ...
        {state === 'loading' && (
          <div className="bg-white rounded-3xl border border-slate-200 p-8 text-center shadow-sm">
            ...
            <div className="text-[14px] text-slate-500">Yükleniyor…</div>
          </div>
        )}
        ...
        {state === 'invalid' && (
          <div className="bg-white rounded-3xl border border-slate-200 p-8 text-center shadow-sm">
            ...
            <div className="text-[17px] font-semibold text-slate-900">Link bulunamadı</div>
            <div className="text-[13.5px] text-slate-500 mt-2">Bu paylaşım linki geçersiz veya silinmiş.</div>
          </div>
        )}
        ...
        {state === 'expired' && (
          <div className="bg-white rounded-3xl border border-slate-200 p-8 text-center shadow-sm">
            ...
            <div className="text-[17px] font-semibold text-slate-900">Link süresi dolmuş</div>
            <div className="text-[13.5px] text-slate-500 mt-2">Bu paylaşım linki artık geçerli değil.</div>
          </div>
        )}
        ...
        {state === 'found' && medicine && st && (
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-6">
              ...
              <h2 className="text-[22px] font-semibold text-slate-900 tracking-tight">{medicine.name}</h2>
              {medicine.quantity && <div className="mt-1 text-[14px] text-slate-500">{medicine.quantity}</div>}

              {ingredients.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {ingredients.map((ing, i) => (
                    <span key={i} className="text-[12px] bg-slate-50 text-slate-700 px-2.5 py-1 rounded-lg ring-1 ring-slate-200">
                      {ing}
                    </span>
                  ))}
                </div>
              ...
              {medicine.notes && (
                <div className="mt-4 text-[13px] text-slate-500 italic">"{medicine.notes}"</div>
              )}
            </div>
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-200">
              <div className="flex items-center justify-between text-[13px] text-slate-600">
                ...
                <span className={`font-semibold ${st.key === 'expired' ? 'text-rose-700' : 'text-slate-900'}`}>
